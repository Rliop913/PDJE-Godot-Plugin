...
{
    ClassDB::bind_method(
		D_METHOD("LoadMusic", "Title", "composer", "bpm"),
		&MusPannelWrapper::LoadMusic);
    ClassDB::bind_method(
		D_METHOD("CueMusic", "title", "newPos"),
		&MusPannelWrapper::CueMusic);
    ClassDB::bind_method(
		D_METHOD("SetMusic", "title", "onoff"),
		&MusPannelWrapper::SetMusic);
    ClassDB::bind_method(
		D_METHOD("GetLoadedMusicList"),
		&MusPannelWrapper::GetLoadedMusicList);
    ClassDB::bind_method(
		D_METHOD("UnloadMusic", "title"),
		&MusPannelWrapper::UnloadMusic);

    ClassDB::bind_method(
		D_METHOD("getFXHandle", "title"),
...
MusPannelWrapper::CueMusic(String title, String newPos)
{
    ...
    try{
        const unsigned long long posull =
        std::stoull(
            GStrToCStr(newPos)
        );
...
MusPannelWrapper::GetLoadedMusicList()
{
    if(musref == nullptr) return Array();
