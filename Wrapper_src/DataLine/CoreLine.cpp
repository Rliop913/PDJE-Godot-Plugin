...
CoreLine::GetUsedFrame()
{
    ...
    return core_data.used_frame ? 
        static_cast<int64_t>(*(core_data.used_frame)) 
        : 
        0;
}
...
CoreLine::GetPreRenderedFrames()
{
    ...
    if (!core_data.maxCursor || !core_data.preRenderedData) {
        WARN_PRINT("GetPreRenderedFrames: null pointer (maxCursor or preRenderedData).");
        return out; // empty
    }
    ...
    // int64_t cursorSize = static_cast<int64_t>(frames) * (uint64_t)PDJE_STEREO_CHANNEL;
    ...
    std::memcpy(dst, core_data.preRenderedData, static_cast<size_t>(total_samples) * sizeof(float));
    return out;
}
