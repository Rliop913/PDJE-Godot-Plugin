...
EditorWrapper::_bind_methods()
{
    ...
    ClassDB::bind_method(D_METHOD("getMixDatas"), &EditorWrapper::getMixDatas);
    ...
    ClassDB::bind_method(
        ...
        &EditorWrapper::Redo);
    ClassDB::bind_method(D_METHOD("Go", "_FLAG_EDITOR_OBJ", "OID"),
                         &EditorWrapper::Go);
    ClassDB::bind_method(D_METHOD("GetLogWithJSONGraph",
                                  "_FLAG_EDITOR_OBJ",
                                  "musicName_if_flag_music"),
                         &EditorWrapper::GetLogWithJSONGraph);
    ClassDB::bind_method(D_METHOD("UpdateLog"), &EditorWrapper::UpdateLog);
...
String
EditorWrapper::Go(const int _FLAG_EDITOR_OBJ, String OID)
{

    if (edit == nullptr)
        return "editor is null";

    auto node_id = GStrToCStr(OID);
    switch (_FLAG_EDITOR_OBJ) {
    case 0:
        if (edit->Go<EDIT_ARG_NOTE>(node_id)) {
            return "OK";
        } else {
            return "NOTE Go Failed";
        }
    case 1:
        if (edit->Go<EDIT_ARG_KEY_VALUE>(node_id)) {
            return "OK";
        } else {
            return "KEY_VALUE Go Failed";
        }
    case 2:
        if (edit->Go<EDIT_ARG_MIX>(node_id)) {
            return "OK";
        } else {
            return "MIX Go Failed";
        }
    case 3:
        if (edit->Go<EDIT_ARG_MUSIC>(node_id)) {
            return "OK";
        } else {
...
EditorWrapper::UpdateLog()
{
    ...
    if (edit == nullptr)
        return "editor is null";
    edit->UpdateLog<EDIT_ARG_MIX>();
    edit->UpdateLog<EDIT_ARG_NOTE>();
    edit->UpdateLog<EDIT_ARG_KEY_VALUE>();
    edit->UpdateLog<EDIT_ARG_MUSIC>();
    return "OK";
}
                       ...
                       String    newNodeID)
{
    ...
    //         Dresult = edit->GetDiff<EDIT_ARG_NOTE>(oldC.value(),
    //         newC.value());
    //     }
    //     break;
    // case 1:
    ...
    //     if (oldC.has_value() && newC.has_value()) {
    //         Dresult =
    //             edit->GetDiff<EDIT_ARG_KEY_VALUE>(oldC.value(),
    //             newC.value());
    //     }
    //     break;
    // case 2:
    ...
    //     CommitSetter(oldC, newC, baseRepo);
    //     if (oldC.has_value() && newC.has_value()) {
    //         Dresult = edit->GetDiff<EDIT_ARG_MIX>(oldC.value(),
    //         newC.value());
    //     }
    //     break;
    // case 3:
    ...
    //     CommitSetter(oldC, newC, baseRepo);
    //     if (oldC.has_value() && newC.has_value()) {
    //         Dresult = edit->GetDiff<EDIT_ARG_MUSIC>(oldC.value(),
    //         newC.value());
    //     }
    //     break;
    // default:
